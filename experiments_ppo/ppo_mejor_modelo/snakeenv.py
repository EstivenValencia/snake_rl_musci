...
class SnakeEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if self.render_mode == 'human':
            ...
            # Display Snake
            cv2.rectangle(self.img,(self.snake_position[0][0],self.snake_position[0][1]),(self.snake_position[0][0]+10,self.snake_position[0][1]+10),(255,0,0),3)
        ...
        eat_self = collision_with_self(self.snake_position) == 1

        # On collision kill the snake and print the score
        if self.truncated or eat_self:
            if self.render_mode == 'human':
                font = cv2.FONT_HERSHEY_SIMPLEX
                self.img = np.zeros((500,500,3),dtype='uint8')
                cv2.putText(self.img,'Your Score is {}'.format(self.score),(140,250), font, 1,(255,255,255),2,cv2.LINE_AA)
                cv2.imshow('a',self.img)
            print("Colisiono con sigo mismo",collision_with_self(self.snake_position))
            print("Colisiono con boundaries",collision_with_boundaries(self.snake_head))
            print("Apple reward: ",apple_reward)
            self.done = True
